 ...
 * Highlights when vitals change significantly.
 */
export function CompactVitalsChip({ vitals, rhythmSummary, onClick }: CompactVitalsChipProps) {
  ...
  // Check if any displayed vital is changing
  const hrChanging = highlightedVitals.has("hr");
  const spo2Changing = highlightedVitals.has("spo2");
  const hasChanging = hrChanging || spo2Changing;
  ...
  return (
    <button
      ...
      } ${hasChanging ? "animate-vital-highlight" : ""}`}
      aria-label={`Vitals: Heart rate ${hr ?? "—"}, SpO2 ${spo2 ?? "—"}%`}
    >
      {/* Heart rate */}
      {hr !== undefined && (
        <span className={`flex items-center gap-1 ${hrAbnormal ? "text-amber-300" : ""} ${hrChanging ? "font-bold" : ""}`}>
      ...
      {/* SpO2 */}
      {spo2 !== undefined && (
        <span className={`flex items-center gap-1 ${spo2Abnormal ? "text-amber-300" : ""} ${spo2Changing ? "font-bold" : ""}`}>
